...
game_on = True
round_init = True
get_next_states(init_state)

def expand(current, _depth=0, _bid_round=0):
    if current.opponent.stack <= 300:
        return current
    elif _depth <= 20:
        print(("  " * _depth), current.opponent.stack)
        children = get_next_states(current)
        for c in children:
            if c.phase == 'BIDDING':
                expand(c, _depth + 1, _bid_round + 1)
            else:
                expand(c, _depth + 1, _bid_round + 0)
    
win_state = expand(init_state)
print(win_state.agent.stack, win_state.opponent.stack)
#  while game_on:
#      if round_init:
#          round_init = False
...
#          states_ = get_next_states(game_state_queue[-1])
#          for i in states_:
#              print(i.phase)
#          print(len(game_state_queue))
#          game_state_queue.extend(states_[:])
#
